...
class ExperimentResourceFactory:
    ...
    @staticmethod
    def create_morph_sets(img_dirs: List[str]) -> Dict[str, Dict[int, StimulusImageFactory]]:
        """
        ...
        image_names = get_img_paths(img_dirs)
        ...
        for image_name in image_names:
            ...
            morph_sets[f"{start_face_id}-{target_face_id}"][int(morph_id)] = \
                StimulusImageFactory(f"{start_face_id}-{target_face_id}-{morph_id}", start_face_id, target_face_id,
                                     morph_id, image_name)
        return morph_sets

    def create_stimulus_image(self, img_filepath: str, img_id: str) -> StimulusImageFactory:
        start_face_id, target_face_id, morph_id = self.get_image_info_from_filepath(img_filepath)
        return StimulusImageFactory(img_id, start_face_id, target_face_id, morph_id, img_filepath)


def get_img_paths_single(img_dir: str) -> list:
    images = []
...
def get_img_paths(img_dirs: List[str]) -> list:
    images = []
    for img_dir in img_dirs:
        images.extend(get_img_paths_single(img_dir))
