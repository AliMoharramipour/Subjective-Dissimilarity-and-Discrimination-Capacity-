...
class SelectionAlgorithm:
    ...
    def __init__(self, init_embedding: np.array, burn_in_iterations: int, run_iterations: int,
                 oracle: typing.Callable, body_selector: typing.Callable, tuple_size=3, verbose=False):
        """
        Args:
            ...
            body_selector (typing.Callable): A body selector function that chooses which tuples to pass to the oracle in each iteration.
    ...
    def burn_in(self, click_data_list: list):
        ...
        for i in range(self.burn_in_iterations):
            for h in range(len(self.init_sim_matrix)):
                candidate_tuple = [h] + list(np.random.choice(self.candidates, self.tuple_size - 1, replace=False))
                while candidate_tuple.count(h) > 1:  # avoid duplicates in burn-in
                    candidate_tuple = [h] + list(np.random.choice(self.candidates, self.tuple_size - 1, replace=False))
                response = self.oracle(candidate_tuple)
    ...
    def selection_algorithm(self, graph_data_list: list, run_data: dict, click_data_list: list,
                            body_selector_data_list: list,
                            n_components=5, n_init=100, max_iter=400, target_matrix=None):
        """Selection algorithm.
        1:
        ...
        burn_in_sorted_tuples = self.burn_in(click_data_list)
        ...
        for i in range(min(max_iterations, self.run_iterations)):
            ...
            graph_data_list.append(deepcopy(run_data))
